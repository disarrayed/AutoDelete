...
local DEFAULT_PROFILE = {
	...
	sellJunk = false,
	sellGreen = true,   -- legacy parity: /sellgreens used to be always-on
	sellBlue = false,
	sellEpic = false,
	...
	for bag = 0, 4 do
		for slot = 1, GetContainerNumSlots(bag) do
			...
			if itemLink and not locked then
				...
				local isGearItem = equipSlot and GEAR_SLOTS[equipSlot]
					and (itemClass == "Armor" or itemClass == "Weapon")
				if vendorPrice and vendorPrice > 0 then
					local shouldSell = false
					local sellReason = nil   -- "list" | "junk" | "green" | "blue" | "epic" | "ilvl"
					...
					-- 2) Quality-based auto-sell. Only runs if not already on the sell list.
					-- Rules per Baenre:
					--   Junk   (q=0): sell unconditionally when sellJunk is on.
					--                 Skips ALL filters (iLvl, BoE, BoE weapons).
					--   Green  (q=2): sell when sellGreen is on AND item is gear
					--                 (armor or weapon slot AND armor/weapon class).
					--   Rare   (q=3): gear only, respects Sell all by iLvl range when enabled.
					--   Epic   (q=4): gear only, respects Sell all by iLvl range when enabled.
					-- "Gear" means both equipSlot matches a gear slot AND itemClass
					-- is Armor or Weapon. The class check protects books/glyphs/
					-- consumables/misc items that might have odd equipSlot values.
					...
					-- 3) Fall-through iLvl rule. Also gear-only: non-gear items
					-- (consumables, books, glyphs, quest items, reagents) are
					-- always protected from iLvl-based auto-sell even if their
					-- iLvl falls inside the range.
					if not shouldSell and ilvlEnabled and ilvl and ilvl >= ilvlMin and ilvl <= ilvlMax
						and isGearItem then
						shouldSell = true; sellReason = "ilvl"
					end

					-- 4) Filters. Applied AFTER the rule above picks a reason, so that a
					-- reason's skip-rules can override global filters. Junk and explicit
					-- sell-list entries skip ALL filters. Green skips BoE-for-armor but
					-- green weapons still respect the BoE weapons iLvl range.
					local isBoE = shouldSell and IsBindOnEquip(bag, slot)

					-- Generic BoE gate: applies to blue/epic/ilvl-fallthrough only.
					-- Skipped for: list, junk, green (green has its own green-weapon path).
