...
-- ============================================================================
-- Style Guide Constants (Baenre UI / PEADAR)
-- ============================================================================
...
local C_PURPLE    = { 0.60, 0.30, 0.75, 1 }   -- #994DBF
local C_RARE_BLUE = { 0.00, 0.44, 0.87, 1 }   -- WoW rare quality blue (#0070DD)
local C_SHAMAN_BLUE = { 0.00, 0.44, 0.87, 1 } -- WoW Shaman class color (#0070DD — same hex as rare blue)
...
local function MakeToggle(parent, label, color, tooltip, tooltipTitle)
	...
	if tooltip then
		...
		row:SetScript("OnEnter", function(self)
			...
			GameTooltip:AddLine(tooltip, C_DIM[1], C_DIM[2], C_DIM[3])
			GameTooltip:Show()
		end)
...
local function MakeDropdown(parent, width, options, onChange)
	...
	-- Button — styled to match ElvUI skinned look (same as pagination buttons):
	-- flat near-black bg, thin black border, subtle 1px top gloss highlight,
	-- teal-tinted border on hover.
	...
	-- Top gloss highlight line (ElvUI's signature accent)
	...
	-- Arrow — uses ElvUI's own arrowup.tga texture (copied to our addon as
	-- elvui_arrow.tga). ElvUI ships a single up-pointing arrow and rotates it
	-- with SetRotation() for other directions. Rotation 3.14 ≈ pi = 180°.
	local arrow = btn:CreateTexture(nil, "OVERLAY", nil, 7)
	arrow:SetTexture("Interface\\AddOns\\AutoDelete\\textures\\arrowup.tga")
	arrow:SetSize(10, 10)
	arrow:SetPoint("RIGHT", -6, 0)
	arrow:SetVertexColor(1, 1, 1, 1)
	arrow:SetRotation(3.14)  -- point DOWN
...
-- ============================================================================
-- Section Helpers (v3.4 sectioned layout)
-- ============================================================================
...
local function MakeSpinnerInput(parent, width, height, minVal, maxVal, onChanged)
	...
	-- Up arrow: ElvUI arrow texture (points up by default, no rotation needed)
	...
	-- Down arrow: same texture rotated 180°
	...
	downGlyph:SetTexture("Interface\\AddOns\\AutoDelete\\textures\\arrowup.tga")
	downGlyph:SetSize(9, 9)
	downGlyph:SetPoint("CENTER", 0, 0)
	downGlyph:SetVertexColor(1, 1, 1, 1)
	downGlyph:SetRotation(3.14)
...
-- MakeSubsectionLabel: small gold tag + label text. More prominent than dim gray
-- since it's the hierarchy anchor inside a section (e.g. "Sell by Rarity",
-- "Item Type" inside Sell Filters).
-- MakeSubsectionLabel: small colored square + label text.
-- iconColor is a 4-element RGBA table; if nil, no icon is drawn.
local function MakeSubsectionLabel(parent, text, xOff, yOff, iconColor)
	local anchorFrame = nil
	if iconColor then
		-- Small colored square (rotated diamond not possible in 3.3.5 — axis-aligned).
		local icon = parent:CreateTexture(nil, "OVERLAY")
		icon:SetTexture(WHITE8x8)
		icon:SetSize(8, 8)
		icon:SetVertexColor(iconColor[1], iconColor[2], iconColor[3], iconColor[4] or 1)
		icon:SetPoint("TOPLEFT", xOff, yOff - 3)
		anchorFrame = icon
	end

	local label = parent:CreateFontString(nil, "OVERLAY")
	label:SetFont(FONT, 10, "OUTLINE")
	label:SetTextColor(0.68, 0.68, 0.68, 1)  -- CSS #aeaeae
	if anchorFrame then
		label:SetPoint("LEFT", anchorFrame, "RIGHT", 8, 0)
	else
		label:SetPoint("TOPLEFT", xOff, yOff - 2)
	end
	label:SetText(text)
...
local function EnsureProfileFields(p)
	-- v3.1 migration: dontSellBoEWeapons → sellBoEWeapons (inverted semantics)
	if p.dontSellBoEWeapons ~= nil then
		p.sellBoEWeapons = not p.dontSellBoEWeapons
		p.dontSellBoEWeapons = nil
	end
	-- v3.2 migration (kept for 3.1→3.3 skip): sellBoEWeaponMinIlvl (sell ≥ floor)
	-- was renamed to sellBoEWeaponMaxIlvl (sell < ceiling). In 3.3, MinIlvl returns
	-- as a genuine range-min, so only migrate if MaxIlvl is still absent (true 3.1 user).
...
local function BuildUI(self)
	...
	-- ========================================================================
	-- v3.4 SECTIONED LAYOUT
	-- Design per Bruce's refined mockup: five titled bordered sections —
	-- General, Sell Filters (with Sell by Rarity + Item Type subsections),
	-- Sell Conditions, Scan Options, Search & Manage. Frame width 500, height 860.
	-- ========================================================================
	-- ========================================================================
	...
	local LEFT_X = LEFT_EDGE                                               -- alias for legacy refs
	...
	-- ========================================================================
	-- SECTION 1: Tabbed Settings (General, Goblin, Tracking, AutoInv, Profiles)
	-- Container panel holds: horizontal tab strip at top + fixed-height content
	-- area below. General tab contains 4 cards: Enable, Summon Scavenger,
	-- Auto-Delete Junk, Scan Speed.
	-- ========================================================================
	...
	local tglEnable = MakeToggle(card1, "Enable", C_ACCENT,
		"Master switch. When enabled, AutoDelete scans your bags and destroys items on the delete list.")
	tglEnable:SetPoint("TOPLEFT", CARD_INNER_PAD_X, -CARD_INNER_PAD_Y)
	tglEnable:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	...
	local tglGray = MakeToggle(card2, "Auto-Delete Junk", C_ACCENT,
		"Automatically destroy poor (gray) quality items without adding them to the delete list.")
	tglGray:SetPoint("TOPLEFT", CARD_INNER_PAD_X, -CARD_INNER_PAD_Y)
	tglGray:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	AddToggleDescription(tglGray, "Permanently delete junk items.",
		cardW - CARD_INNER_PAD_X * 2 - 26)
	...
	-- CARD 1: Auto-Repair (main toggle + "Use Guild Bank" sub-toggle)
	...
	tglRepair:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	self._tglRepair = tglRepair

	-- Sub-toggles indent to align their box with the parent toggle's TEXT.
	-- Parent anchor = CARD_INNER_PAD_X (10), parent box = 18 wide, gap 8 → text starts at 36.
	local SUBTGL_INDENT = CARD_INNER_PAD_X + 16 + 8   -- 34 (card pad + parent box + gap)
	local tglRepairGuild = MakeSubToggle(gCard1, "Use Guild Bank money", C_DK_RED)
	tglRepairGuild:SetPoint("TOPLEFT", SUBTGL_INDENT, -(CARD_INNER_PAD_Y + 24))
	...
	local tglScav = MakeToggle(gCard2, "Summon Scavenger", C_ACCENT,
		"Automatically summon your Greedy Scavenger companion pet after vendoring.")
	tglScav:SetPoint("TOPLEFT", CARD_INNER_PAD_X, -CARD_INNER_PAD_Y)
	tglScav:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	self._tglScav = tglScav

	local tglScavAfterSell = MakeSubToggle(gCard2, "After sell", C_DK_RED)
	tglScavAfterSell:SetPoint("TOPLEFT", SUBTGL_INDENT, -(CARD_INNER_PAD_Y + 24))
	...
	tglScavAfterClose:SetPoint("TOPLEFT", SUBTGL_INDENT, -(CARD_INNER_PAD_Y + 42))
	tglScavAfterClose:SetWidth(cardW - SUBTGL_INDENT - CARD_INNER_PAD_X)
	self._tglScavAfterClose = tglScavAfterClose

	-- CARD 3: Hide Greedy Spam (single toggle for chat + bubbles together)
	local tglHideSpam = MakeToggle(gCard3, "Hide Greedy Spam", C_ACCENT,
		"Hides Greedy Scavenger's chat messages and speech bubbles.")
	tglHideSpam:SetPoint("TOPLEFT", CARD_INNER_PAD_X, -CARD_INNER_PAD_Y)
	tglHideSpam:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	AddToggleDescription(tglHideSpam, "Silence the scavenger's chat & bubbles.",
		cardW - CARD_INNER_PAD_X * 2 - 26)
	...
	tglAutoInvite:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	...
	tglLootRule:SetSize(cardW - CARD_INNER_PAD_X * 2, 22)
	...
	-- Layout:
	--   [Current: Baenre               ]   ← header at top
	--   Profile: [dropdown         v]       ← dropdown picks a target char
	--   [Copy]  [Delete]  [Reset Current]   ← 3 action buttons
	--
	-- Buttons operate on the dropdown selection for Copy/Delete. Reset always
	-- operates on the current character. Every destructive action pops a
	-- StaticPopup confirmation. The dropdown includes the current character
	-- too (useful for "reset this char's settings via delete").
	-- ========================================================================
	do
		...
		-- Action buttons. Styled like the Reset Stats button on Tracking tab
		-- so the whole addon feels uniform.
	...
	-- ========================================================================
	-- SECTION 2: Sell Filters (auto-sized from helper text / last element)
	--   TOP_PAD        → rarity subsection label at y=-TOP_PAD            (12px tall)
	--   subsection+gap → rarity row at y=-28                               (22px tall)
	--   rarity_end     = -50
	--   divider        at y=-54
	--   item type lbl  at y=-60                                            (12px tall)
	--   BoE toggle     at y=-76                                            (22px tall)
	--   BoE desc       at y=-99                                            (14px tall, 1 line)
	--   content bottom = -113
	--   BOT_PAD        → section height = 119
	-- ========================================================================
	local RARITY_Y = -28
	local DIVIDER_Y = -54
	local ITEM_TYPE_LBL_Y = -60
	local BOE_TOGGLE_Y = -76
	local filtersBox = MakeSection(self, "Sell Filters", yOff, 1)

	-- Subsection: Sell by Rarity
	MakeSubsectionLabel(filtersBox, "Sell by Rarity", 10, -TOP_PAD, C_SHAMAN_BLUE)

	local RARITY_LEFT = 20
	local RARITY_SPAN = CONTENT_W - RARITY_LEFT - 15
	-- 4 equal columns — each toggle gets the same column width so the LEFT edges
	-- are evenly spaced regardless of label length.
	local colW_rarity = math.floor(RARITY_SPAN / 4)
	local W_JUNK, W_UNC, W_RARE, W_EPIC = colW_rarity, colW_rarity, colW_rarity, colW_rarity

	local tglSellJunk = MakeToggle(filtersBox, "Junk", C_Q_POOR,
		"Sell poor (gray) quality items at vendors. Applies to items in any slot, not just gear.")
	tglSellJunk:SetPoint("TOPLEFT", RARITY_LEFT, RARITY_Y)
	tglSellJunk:SetSize(W_JUNK, 22)
	self._tglSellJunk = tglSellJunk

	local tglSellGreen = MakeToggle(filtersBox, "Uncommon", C_Q_UNCOMMON,
		"Sell Uncommon (green) gear at vendors. Gear slots only.")
	tglSellGreen:SetPoint("TOPLEFT", RARITY_LEFT + colW_rarity, RARITY_Y)
	tglSellGreen:SetSize(W_UNC, 22)
	self._tglSellGreen = tglSellGreen

	local tglSellBlue = MakeToggle(filtersBox, "Rare", C_Q_RARE,
		"Sell Rare (blue) gear at vendors. Gear slots only. When \"Sell all items by iLvl\" is enabled, items must also fall inside that iLvl range to be sold.")
	tglSellBlue:SetPoint("TOPLEFT", RARITY_LEFT + colW_rarity * 2, RARITY_Y)
	tglSellBlue:SetSize(W_RARE, 22)
	self._tglSellBlue = tglSellBlue

	local tglSellEpic = MakeToggle(filtersBox, "Epic", C_Q_EPIC,
		"Sell Epic (purple) gear at vendors. Gear slots only. When \"Sell all items by iLvl\" is enabled, items must also fall inside that iLvl range to be sold.")
	tglSellEpic:SetPoint("TOPLEFT", RARITY_LEFT + colW_rarity * 3, RARITY_Y)
	tglSellEpic:SetSize(W_EPIC, 22)
	self._tglSellEpic = tglSellEpic

	-- Thin divider between subsections
	local subDivider = filtersBox:CreateTexture(nil, "ARTWORK")
	subDivider:SetTexture(WHITE8x8)
	subDivider:SetHeight(1)
	subDivider:SetPoint("TOPLEFT", 10, DIVIDER_Y)
	subDivider:SetPoint("TOPRIGHT", -10, DIVIDER_Y)
	subDivider:SetVertexColor(0.12, 0.12, 0.12, 1)

	-- Subsection: Item Type
	MakeSubsectionLabel(filtersBox, "Item Type", 10, ITEM_TYPE_LBL_Y, C_SHAMAN_BLUE)

	local tglBoE = MakeToggle(filtersBox, "Sell BoE items", C_ACCENT,
		"Also sell Bind-on-Equip items at vendors. When disabled, any item that says \"Binds when equipped\" is protected from being sold.")
	tglBoE:SetPoint("TOPLEFT", 20, BOE_TOGGLE_Y)
	tglBoE:SetSize(140, 22)
	AddToggleDescription(tglBoE, "Only items that are Bind on Equip will be sold.", CONTENT_W - 60)
	self._tglBoE = tglBoE

	-- AUTO-SIZE: BoE description is the lowest element
	local filtersActualH = AutoSizeSection(filtersBox, tglBoE._desc, 8, 120)
	yOff = yOff - 16 - filtersActualH - SECTION_GAP

	-- ========================================================================
	-- SECTION 3: Sell Conditions (auto-sized)
	-- Row 1 at -TOP_PAD; Row 2 ROW_GAP below row 1 top-to-top; Info below row 2.
	-- ========================================================================
	local ROW1_Y = -TOP_PAD            -- = -12
	local ROW_GAP_COND = 22            -- top-to-top (rows are 22 tall → 0px visual gap; prev 24 had 2px)
	local ROW2_Y = ROW1_Y - ROW_GAP_COND   -- = -34
	local INFO_GAP = 6                 -- gap between row 2 bottom and info top
	local INFO_Y = ROW2_Y - 22 - INFO_GAP  -- = -62
	local condBox = MakeSection(self, "Sell Conditions", yOff, 1)

	local COND_LABEL_X = 15
	local COND_INPUT_X = 240
	local COND_INPUT_GAP = 10

	-- Row 1: Sell BoE weapons by iLvl
	local tglBoEWeapons = MakeToggle(condBox, "Sell BoE weapons by iLvl:", C_ACCENT,
		"When \"Sell BoE items\" is also enabled, sell Bind-on-Equip weapons whose item level falls inside this range (inclusive). Weapons outside the range are protected.")
	tglBoEWeapons:SetPoint("TOPLEFT", COND_LABEL_X, ROW1_Y)
	tglBoEWeapons:SetSize(190, 22)
	self._tglBoEWeapons = tglBoEWeapons

	local SPINNER_W = 74  -- wider than plain 60 to make room for arrow buttons

	local boeMinFrame = MakeSpinnerInput(condBox, SPINNER_W, 22, 1, 9999)
	boeMinFrame:SetPoint("LEFT", tglBoEWeapons, "RIGHT", 30, 0)
	local boeMinBox = boeMinFrame.editBox
	self._boeMinBox = boeMinBox

	local boeToLabel = MakeText(condBox, 9, C_DIM, "OUTLINE")
	boeToLabel:SetPoint("LEFT", boeMinFrame, "RIGHT", COND_INPUT_GAP, 0)
	boeToLabel:SetText("to")

	local boeMaxFrame = MakeSpinnerInput(condBox, SPINNER_W, 22, 1, 9999)
	boeMaxFrame:SetPoint("LEFT", boeToLabel, "RIGHT", COND_INPUT_GAP, 0)
	local boeMaxBox = boeMaxFrame.editBox
	self._boeMaxBox = boeMaxBox

	-- Row 2: Sell all items by iLvl
	local tglIlvl = MakeToggle(condBox, "Sell all items by iLvl:", C_ACCENT,
		"Sell any equippable gear with an item level inside this range (inclusive), regardless of quality. Gear slots only. Non-gear items (consumables, books, reagents) are never affected by this rule. Also acts as a gate for the Rare and Epic sell toggles. When this is enabled, those tiers only sell items within this range.")
	tglIlvl:SetPoint("TOPLEFT", COND_LABEL_X, ROW2_Y)
	tglIlvl:SetSize(190, 22)
	self._tglIlvl = tglIlvl

	local minFrame = MakeSpinnerInput(condBox, SPINNER_W, 22, 1, 9999)
	minFrame:SetPoint("LEFT", tglIlvl, "RIGHT", 30, 0)
	local minBox = minFrame.editBox
	self._minBox = minBox

	local toLabel = MakeText(condBox, 9, C_DIM, "OUTLINE")
	toLabel:SetPoint("LEFT", minFrame, "RIGHT", COND_INPUT_GAP, 0)
	toLabel:SetText("to")

	local maxFrame = MakeSpinnerInput(condBox, SPINNER_W, 22, 1, 9999)
	maxFrame:SetPoint("LEFT", toLabel, "RIGHT", COND_INPUT_GAP, 0)
	local maxBox = maxFrame.editBox
	self._maxBox = maxBox

	-- Info note at bottom of section: lowercase "i" drawn in a 16x16 bordered
	-- "badge". Not a true circle (3.3.5 can't do rounded corners without a
	-- custom texture), but the proportions + dim palette read as an info badge.
	local infoBadge = CreateFrame("Frame", nil, condBox)
	infoBadge:SetSize(16, 16)
	infoBadge:SetPoint("TOPLEFT", 15, INFO_Y)
	ApplyBackdrop(infoBadge, { 0.12, 0.12, 0.12, 1 }, { C_DIM[1], C_DIM[2], C_DIM[3], 1 })
	local infoGlyph = infoBadge:CreateFontString(nil, "OVERLAY")
	infoGlyph:SetFont(FONT, 11, "OUTLINE")
	infoGlyph:SetTextColor(C_DIM[1] + 0.15, C_DIM[2] + 0.15, C_DIM[3] + 0.15, 1)
	infoGlyph:SetPoint("CENTER", 0, 0)
	infoGlyph:SetText("i")
	local infoText = MakeText(condBox, 10, C_DIM, "OUTLINE")
	infoText:SetPoint("LEFT", infoBadge, "RIGHT", 8, 0)
	infoText:SetText("Items must match the selected rarity and above conditions to be sold.")

	-- AUTO-SIZE: infoBadge is the lowest element (infoText is anchored to its RIGHT so same y-bottom)
	local condActualH = AutoSizeSection(condBox, infoBadge, 8, 90)
	yOff = yOff - 16 - condActualH - SECTION_GAP

	-- ========================================================================
	-- SECTION 4: Scan Options — Delete/Sell/Keep list mode tabs
	-- Scan Speed moved to the General settings tab; this section now contains
	-- only the list mode selector buttons.
	-- ========================================================================
	...
	-- ========================================================================
	-- SECTION 5: Search & Manage (search row + item list)
	-- Remaining vertical space gets used for this section; derived from actual frame height.
	-- ========================================================================
	local bottomPad = 15
	...
	-- Small helper for a single nav button styled to match ElvUI's skinned
	-- button look: flat black background, thin black border, a subtle 1px
	-- highlight line along the top edge (the ElvUI "glossy" accent), and a
	-- hover state that lifts the background slightly and tints the text.
	--
	-- If `texturePath` is provided, draws a texture (arrow) instead of text.
	-- `rotation` is in radians (pi = 180°, pi/2 = 90°).
	local function MakePageBtn(label, w, fontSize, texturePath, texW, texH, rotation)
		...
		-- Thin top highlight line (ElvUI's signature subtle gloss).
		local gloss = btn:CreateTexture(nil, "OVERLAY")
		gloss:SetTexture(WHITE8x8)
		gloss:SetVertexColor(1, 1, 1, 0.08)
		gloss:SetHeight(1)
		...
		if texturePath then
			-- Single texture, rotated per direction.
	...
	-- All pagination buttons use ElvUI's arrow texture, rotated per direction.
	-- Pagination: single arrow for prev/next, double-arrow texture for first/last.
	-- Rotations: pi/2 (1.57) = left (default arrow points up), -pi/2 (-1.57) = right.
	local SINGLE_TEX = "Interface\\AddOns\\AutoDelete\\textures\\arrowup.tga"
	local DOUBLE_TEX = "Interface\\AddOns\\AutoDelete\\textures\\doublearrowup.tga"
	local firstBtn = MakePageBtn(nil, 28, nil, DOUBLE_TEX, 12, 12,  1.57)   -- <<
	local prevBtn  = MakePageBtn(nil, 28, nil, SINGLE_TEX, 12, 12,  1.57)   -- <
	local nextBtn  = MakePageBtn(nil, 28, nil, SINGLE_TEX, 12, 12, -1.57)   -- >
	local lastBtn  = MakePageBtn(nil, 28, nil, DOUBLE_TEX, 12, 12, -1.57)   -- >>
	...
	tglBoE:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellBoE = btn._checked
	end)

	tglBoEWeapons:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellBoEWeapons = btn._checked
		if btn._checked then
			boeMinBox:SetTextColor(1, 1, 1, 1)
			boeMaxBox:SetTextColor(1, 1, 1, 1)
		else
			boeMinBox:SetTextColor(0.5, 0.5, 0.5, 1)
			boeMaxBox:SetTextColor(0.5, 0.5, 0.5, 1)
		end
	end)

	tglSellJunk:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellJunk = btn._checked
	end)

	tglSellGreen:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellGreen = btn._checked
	end)

	tglSellBlue:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellBlue = btn._checked
	end)

	tglSellEpic:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellEpic = btn._checked
	end)
	...
	tglIlvl:SetScript("OnClick", function(btn)
		btn._checked = not btn._checked
		btn:SetChecked(btn._checked)
		GetActiveProfile(db).sellIlvlEnabled = btn._checked
	end)

	minBox:SetScript("OnTextChanged", function(eb)
		local val = tonumber(eb:GetText())
		if val then GetActiveProfile(db).sellIlvlMin = val end
	end)
	maxBox:SetScript("OnTextChanged", function(eb)
		local val = tonumber(eb:GetText())
		if val then GetActiveProfile(db).sellIlvlMax = val end
	end)
	boeMinBox:SetScript("OnTextChanged", function(eb)
		local val = tonumber(eb:GetText())
		if val and val > 0 then GetActiveProfile(db).sellBoEWeaponMinIlvl = val end
	end)
	boeMaxBox:SetScript("OnTextChanged", function(eb)
		local val = tonumber(eb:GetText())
		if val and val > 0 then GetActiveProfile(db).sellBoEWeaponMaxIlvl = val end
	end)
	...
	function self:Refresh()
		...
		tglBoE:SetChecked(p.sellBoE)
		tglBoEWeapons:SetChecked(p.sellBoEWeapons)
		tglSellJunk:SetChecked(p.sellJunk)
		tglSellGreen:SetChecked(p.sellGreen)
		tglSellBlue:SetChecked(p.sellBlue)
		tglSellEpic:SetChecked(p.sellEpic)
		...
		tglIlvl:SetChecked(p.sellIlvlEnabled)
		speedDD:SetValue((p.scanInterval and p.scanInterval >= 0.5) and p.scanInterval or 0.5)
		minBox:SetText(tostring(p.sellIlvlMin or 1))
		maxBox:SetText(tostring(p.sellIlvlMax or 199))
		boeMinBox:SetText(tostring(p.sellBoEWeaponMinIlvl or 1))
		boeMaxBox:SetText(tostring(p.sellBoEWeaponMaxIlvl or 199))
		-- Dim both BoE inputs when the BoE weapons toggle is off
		if p.sellBoEWeapons then
			boeMinBox:SetTextColor(1, 1, 1, 1)
			boeMaxBox:SetTextColor(1, 1, 1, 1)
		else
			boeMinBox:SetTextColor(0.5, 0.5, 0.5, 1)
